...
class ROLO_TF:
    ...
    rolo_model_file = 'panchen/output/ROLO_model_4/'
    weights_file = os.path.join(rolo_model_file, 'model_step4_exp1.ckpt')
    lstm_depth = 3
    ...
    def train_20(self):
        ...
        with tf.name_scope('loss'):
            ...
            self.lstm_pred = tf.square(fw_prediction-bw_prediction)
            self.accuracy = (tf.reduce_mean(self.correct_prediction)
                             +tf.reduce_mean(self.lstm_pred))* 100
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(epoches):  # 22*50
                log_file = open('panchen/output/train_model_4.txt', 'a')
